...
export default function AdminEditLessonPage() {
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        const [courseData, topicData, lessonData] = await Promise.all([
        ...
        if (lessonData) {
          ...
          setType(lessonData.type || 'VOCABULARY');
          setOrderIndex(lessonData.orderIndex || 0);

          let obj: any = {};
          try {
            obj = typeof lessonData.contentJson === 'string' ? JSON.parse(lessonData.contentJson) : lessonData.contentJson;
          } catch {
            obj = {};
          }

          if (obj) {
            setVocabWord(obj.word || '');
            setVocabPronunciation(obj.pronunciation || '');
            setVocabMeaning(obj.meaning || '');
            setVocabExample(obj.example || '');

            setFillSentence(obj.sentence || '');
            setFillAnswer(obj.answer || '');

            setShadowAudioText(obj.audioText || '');

            setSituationText(obj.situation || '');
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-2 text-sm text-slate-400">
        ...
        <ChevronRight size={14} className="text-slate-600" />
        <span className="text-white font-semibold truncate max-w-xs">{title || 'Chỉnh sửa bài làm'}</span>
      </div>
