...
export const startConversation = async (apiKey: string, topic: string, level: string = 'B1', modelId: string = 'gemini-1.5-pro') => {
    const model = getGeminiModel(apiKey, modelId);
    const chat = model.startChat({
        history: [
            {
                role: 'user',
                parts: [{ text: `We are going to have a conversation in English about the topic: "${topic}". You act as my English conversation partner. My English proficiency level is ${level}. Please adapt your vocabulary, grammar complexity, and response length to match my level.
